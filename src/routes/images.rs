...
impl ImagesApi {
    ...
    async fn get_image(
        ...
        let response = match get_object_request
            .send()
            .await
    ...
    async fn put_image(
        ...
        let put_object_request = object_storage.put_object(IMAGES_FILE_BUCKET, &*name, SegmentedBytes::from(Bytes::from(contents)));

        put_object_request.send().await.map_err(InternalServerError)?;
